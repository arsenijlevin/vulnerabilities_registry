...
import { useChatbotFormSubmit } from '@/hooks/useChatbotFormSubmit';

type RecordType = string | number | boolean | null;
export type TableDataType = Record<string, RecordType>;
...
export default function CustomPaginationActionsTable<T extends TableDataType>({
  ...
  const data = useChatbotFormSubmit();
        ...
        <TableFooter>
          <TableRow>
            <TablePagination
              ...
              labelDisplayedRows={({ from, to, count }) => `${from.toString()}-${to.toString()} из ${count.toString()}`}
...
function TableHeader({ headers, withDescription }: TableHeaderProps) {
  ...
  withDescription &&
    additionalHeaders.unshift({
      title: 'Описание',
      key: 'descriptionButton',
    });
